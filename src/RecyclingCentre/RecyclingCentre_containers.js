...
export default class RecyclingCentre extends MiniGameUi {
  ...
  startGame() {
    ...
    this.platform2 = this.matter.add.gameObject(
        this.add.rectangle(80, 100, 170, 5, 0xffffff).setOrigin(0.5, 0.5),
        { isStatic: true }
      );

    this.platform3 = this.matter.add.gameObject(
        this.add.rectangle(500, 100, 170, 5, 0xffffff).setOrigin(0.5, 0.5),
        { isStatic: true }
    );

    this.containers.push(this.matter.add.gameObject(
  ...
  update() {
    ...
    if (this.currentObject && this.currentObject.y < 60) {
